...
2. It smashes all the include headers into a single header [include/PotatoDialect.h](include/PotatoDialect.h) and all the tablegen into a single [include/PotatoDialect.td](include/PotatoDialect.td) and *also emits all the tablegen into the source tree itself*;
   1. I think seeing the emitted tablegen is useful for demystifying how MLIR works
3. It smashes all the implementation into a single [src/PotatoDialect.cpp](src/PotatoDialect.cpp)
...
A potato CMake might look like:
...
and then just do `pushd build && ninja check-potato && popd`.
